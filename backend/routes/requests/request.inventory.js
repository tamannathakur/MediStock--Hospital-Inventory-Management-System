...
router.put("/:id/approve-inventory", auth, authorize(["inventory_staff"]), async (req, res) => {
  try {
    ...
    // =========================================================
    // 🛑 Case 1 — Store Request (Multi-Item)
    // =========================================================
    if (request.requestType === "store_request") {
      if (!Array.isArray(request.items) || request.items.length === 0) {
        return res.status(400).json({ msg: "Store request is missing items" });
      ...
      // 1. Validation Loop: Check if we have enough stock for EVERYTHING first
      // We do NOT check vendorStatus here. We trust the Product DB numbers.
      for (const item of request.items) {
        const product = await Product.findOne({ name: item.productName });

        if (!product) {
          return res.status(400).json({ msg: `Product '${item.productName}' not found in database.` });
        }

        // If actual stock is less than requested, THEN we know vendor hasn't delivered
        if (product.totalQuantity < item.quantity) {
          return res.status(400).json({ 
            msg: `Vendor has not delivered yet. Insufficient stock for: ${item.productName} (Have: ${product.totalQuantity}, Need: ${item.quantity})` 
          });
        }
      }

      // 2. Execution Loop: If we passed the loop above, we have stock! Deduct and Dispatch.
      for (const item of request.items) {
        const product = await Product.findOne({ name: item.productName });
      ...
      // Final Status Update
      request.status = "approved_and_sent";
      request.vendorStatus = "stored"; // Implicitly stored since we just sent it
      await request.save();

      return res.json({ msg: "Store request items deducted from Central and dispatched.", request });
    }

    // =========================================================
    // 🟢 Case 2 — Normal Requests (Single Item)
    // =========================================================
    if (!request.product) return res.status(400).json({ msg: "Request missing product reference" });
    ...
    if (product.totalQuantity < request.quantity) {
      return res.status(400).json({ 
        msg: `Vendor has not delivered yet. Insufficient stock. (Have: ${product.totalQuantity}, Need: ${request.quantity})` 
      });
    }
